...
セッション選択・変換・プレビュー・ダウンロードを一画面で行える Web UI。

![Web UI スクリーンショット](docs/media/screenshot1.png)

### Terminal プレイヤー（デモ）

Codex CLI セッションを Terminal レンダラーで再生した様子（アニメーション GIF）。

![Terminal プレイヤーのデモ GIF](docs/media/codex-terminal-1-35-10s.gif)

高画質版の動画: [codex-terminal-1-35.mp4](docs/media/codex-terminal-1-35.mp4)
...
```mermaid
flowchart TB
    S1["Stage 1: Capture (Agent Adapters)<br/>各エージェントのログを読み取り、共通モデルに変換<br/>claude-log2model.py / codex-log2model.py / ..."]
